...
export default function CaloriesContentHub() {
  return <section className="calorie-content-hub" aria-labelledby="calorie-hub-title"><div className="container container-article">
    ...
    <aside className="nutrition-data-warning"><strong>وضعیت اعتبار داده:</strong> همه اقلام مجموعه فعلی با برچسب داخلی «منبع تأییدنشده» نگهداری می‌شوند و تاریخ بازبینی منبع ندارند.</aside>
    ...
    <section><h3>ادامه در یخچال</h3><p>برای برنامه‌ریزی وعده‌ها و آماده‌کردن فهرست خرید، <Link href="/features/meal-planner">برنامه غذای هفته</Link> را ببینید یا به <Link href="/download">صفحه دانلود اپلیکیشن</Link> بروید.</p></section>
  </div></section>;
}
